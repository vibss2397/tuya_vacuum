...
class TestBrainSaveRoom(unittest.TestCase):
    ...
    def test_save_simple_rectangle(self):
        ...
        self.assertEqual(self.world_map.get_cell(1, 1).room_name, "bedroom")
    ...
    def test_save_subsequent_room(self):
        ...
        # After room2 is saved, overlapping cells belong to room2
        room1_cells = self.world_map.get_all_cells_for_room("room1")
        room2_cells = self.world_map.get_all_cells_for_room("room2")

        # room1 now only has cells that weren't overwritten: (0, 0) and (0, 1)
        self.assertEqual(len(room1_cells), 2)
        # room2 has 4 cells: (1, 0), (1, 1), (2, 0), (2, 1)
        self.assertEqual(len(room2_cells), 4)

        # Overlapping cells belong to room2 (last assigned)
        self.assertEqual(self.world_map.get_cell(1, 1).room_name, "room2")
        self.assertEqual(self.world_map.get_cell(1, 0).room_name, "room2")

        # Non-overlapping room1 cells still belong to room1
        self.assertEqual(self.world_map.get_cell(0, 0).room_name, "room1")
        self.assertEqual(self.world_map.get_cell(0, 1).room_name, "room1")
